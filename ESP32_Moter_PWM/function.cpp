...
void getRMS() {
  ...
  acs712Voltage = voltageSum / sampleCount;      // V RMS값 계산
...
void getPeakRMS() {
  ...
  float rmsVoltage = 0;     // RMS 전압 값 저장
  float peakAmpere = 0;     // 피크 전류 값 저장
  float rmsAmpere = 0;      // RMS 전류 값 저장
  ...
  // 피크 값을 전압으로 변환
  float peakVoltage = getVoltage(peakValue);  // ADC 값을 전압으로 변환

  // 피크 전압으로부터 피크 전류 계산
  peakAmpere = (peakVoltage - offsetVoltage) / sensitivity;  // 오프셋 보정 및 감도 적용

  // 피크값으로 RMS 값 계산 (사인파 가정)
  rmsVoltage = peakVoltage * 0.707;  // 피크 전압에서 RMS 전압으로 변환
  rmsAmpere = peakAmpere * 0.707;    // 피크 전류에서 RMS 전류로 변환

  // 결과 출력
  Serial.print("피크 값 RMS 계산(근사) - 전압: ");
  Serial.print(rmsVoltage);
  Serial.println("V");
  Serial.print("피크 값 RMS 계산(근사) - 전류: ");
  Serial.print(rmsAmpere);
  Serial.println("A");
}