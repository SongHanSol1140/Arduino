...
#include "esp_system.h" // ets_printf 함수를 위한 헤더 파일
#else
#include "md5.h"
#endif
...
static bool getMD5(uint8_t * data, uint16_t len, char * output) { //33 bytes or more
  ...
  mbedtls_md5_starts(&_ctx); // _ret 제거
  mbedtls_md5_update(&_ctx, data, len); // _ret 제거
  mbedtls_md5_finish(&_ctx, _buf); // _ret 제거
