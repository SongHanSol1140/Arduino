...
float measureCurrentRMS() {
  ...
  for (int i = 0; i < numSamples; i++) {
    adcValue = analogRead(currentSensorPin);  // 전류 센서의 ADC 값 읽기
...
void loop() {
  ...
  // Serial.println(adcValue / 4095.0 * 3.3);
  // Serial.print("VCC : ");
  // Serial.println(vccCheck);
  // RMS 전류 측정
